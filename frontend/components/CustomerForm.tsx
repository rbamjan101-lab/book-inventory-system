...
import type { Customer, CustomerCategory, CustomerCreate, CustomerUpdate } from '@/lib/api';

type Props = {
        initial?: Partial<Customer>;
        submitLabel?: string;
        onSubmit: (values: CustomerCreate | CustomerUpdate) => Promise<void> | void;
};

const emailPattern = /.+@.+\..+/;

const categoryOptions: { label: string; value: CustomerCategory }[] = [
        { label: 'School', value: 'school' },
        { label: 'Stationery Shop', value: 'stationery_shop' },
        { label: 'Dealer', value: 'dealer' },
];

export default function CustomerForm({ initial, submitLabel = 'Save', onSubmit }: Props) {
        ...
        const [category, setCategory] = useState<CustomerCategory>(initial?.category ?? 'school');
        ...
        return (
                <form onSubmit={handleSubmit} className="space-y-4">
                        <div className="grid gap-4 sm:grid-cols-2">
                                ...
                                <label className="flex flex-col gap-1">
                                        <span className="text-sm font-medium text-slate-600">Customer Category</span>
                                        <select
                                                value={category}
                                                onChange={(event) => setCategory(event.target.value as CustomerCategory)}
                                                className={`rounded-md border px-3 py-2 text-sm ${
                                                        fieldErrors.category ? 'border-red-500' : 'border-slate-200'
                                                }`}
                                        ...
                                        {fieldErrors.category && <span className="text-xs text-red-600">{fieldErrors.category}</span>}
                                </label>
                                <label className="flex flex-col gap-1 sm:col-span-2">
                                        <span className="text-sm font-medium text-slate-600">Billing Address</span>
                                        <textarea
